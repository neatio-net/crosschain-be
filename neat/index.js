...
const logger = require('../logger').child({component: "neatio"})
...
async function setNonce() {
    try {
        ...
        const msg = "The neatio local nonce has has been set"
        logger.info(msg);
        console.log(msg)
    } catch (error) {
        const msg = `Error while trying to set the neatio local nonce: ${error}`
        logger.error(msg);
        console.error(msg)
        throw error
    }
...
exports.send = async function (address, amount, takeFee) {
    try {
        ...
        if (nonce !== null && epoch !== null) {
            if (takeFee) {
                amount = parseFloat(amount) - parseFloat(process.env.NEATIO_FIXED_FEES)
            } else {
                amount = parseFloat(amount)
            }
            logger.info(`Sending neatio tx, address: ${address}, amount: ${amount}, epoch: ${epoch}, nonce: ${nonce}`)
            ...
            return {
                ...
                fees: parseFloat(process.env.NEATIO_FIXED_FEES),
...
exports.isTxConfirmed = async function (transaction) {
    try {
        ...
        return bcn_syncing.data.result.highestBlock > bcn_block.data.result.height + parseInt(process.env.NEATIO_CONFIRMATIONS_BLOCKS) || false
    } catch (error) {
...
exports.isValidSendTx = async function (transaction, address, amount, date) {
    ...
    try {
        ...
        const recipient = privateKeyToAddress(process.env.NEATIO_PRIVATE_KEY)
        ...
        logger.error(`Failed to check if neatio tx is valid: ${error}`);
        return false
    }
}
...
exports.getWalletAddress = function () {
    return privateKeyToAddress(process.env.NEATIO_PRIVATE_KEY);
}

exports.isNewTx = async function (tx) {
    try {
        const [data] = await db.promise().execute("SELECT `id` FROM `used_txs` WHERE `tx_hash` = ? AND `blockchain` = 'neatio';", [tx]);
        if (data[0]) {
            return false
        } else {
            return true
