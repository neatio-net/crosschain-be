...
exports.estimateMint = async function (address, amount) {
    try {
        ...
        const neatPrice = await getNeatPrice();
        if (!neatPrice) {
            return null
        }
        const fees = ethers.utils.parseUnits((await provider.getGasPrice() * await contract.estimateGas.mint(address, amount) / neatPrice).toString(), 'ether').div(ethers.BigNumber.from(100)).mul(ethers.BigNumber.from(process.env.BSC_FEES));
        return {
            contract: contract,
            amount: amount,
            fees: fees
...
async function getNeatPrice() {
    let resp = await axios.get("https://api.coingecko.com/api/v3/simple/price?ids=neatio&vs_currencies=bnb");
    if (resp.status !== 200) {
        logger.error(`Failed to get Neatio price, status: ${resp.status}`);
        return 0
    }
    if (!resp.data.neatio.bnb) {
        logger.error(`Failed to get Neatio price, res: ${resp.data.neatio.bnb}`);
        return 0
    }
    return ethers.utils.parseEther(resp.data.neatio.bnb.toString());
}
...
exports.calculateFees = async function (address, amount) {
    try {
        ...
        let neatPrice = await getNeatPrice();
        if (!neatPrice) {
            return {}
        }
        let fees = ethers.utils.parseUnits((await provider.getGasPrice() * await contract.estimateGas.mint(address, amount) / neatPrice).toString(), 'ether').div(ethers.BigNumber.from(100)).mul(ethers.BigNumber.from(process.env.BSC_FEES));
